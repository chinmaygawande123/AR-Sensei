...
public class SpeechManager : MonoBehaviour {
    ...
    private void Awake()
    {
	    ...
	    voiceName = VoiceName.enUSSaraNeural;
    }
    ...
    private static float[] FixedRAWAudioToUnityAudio(byte[] wavAudio, int channelCount, int resolution, out int sampleCount)
	{
        ...
        if (channelCount == 2) { sampleCount /= 2; }  // 4 bytes per sample (16 bit stereo)
        Debug.Log($"Audio data contains {sampleCount} samples. Starting conversion");
	    File.WriteAllBytes(SerialK + "Downloaded.mp3", wavAudio);
	    print("Downloede");
        // Allocate memory (supporting left channel only)
        var unityData = new float[sampleCount];
    ...
    private IEnumerator WaitAndPlayRoutineSDK(Task<SpeechSynthesisResult> speakTask)
    {
        ...
        if (result.Reason == ResultReason.SynthesizingAudioCompleted)
        {
	        ...
	        if(AudioClips.Count < 4)
	        {
	        	clip.name = AudioClips.Count + "clip";
		        AudioClips.Add(clip);
		        SpeechTeller.SpeechStatus++;
		        SpeechTeller.SpeechPlayback();
	        }
	        ...
	        if(FirstDownload == 0)
	        {
	        	...
	        	audioSource.Play();
	        	InvokeRepeating("WaitingForFinishing", 0, 0.1f);
	        	FirstDownload = 1;
	        }
	...
	void WaitingForFinishing()
	{
		if(!audioSource.isPlaying)
		{
			if(audioSource.clip.name == "0clip")
			{
				StartCoroutine(WaitforTheClip(1f, 1));
			}
			
			CancelInvoke("WaitingForFinishing");
		}
	}
	
	IEnumerator WaitforTheClip(float timeforclip, int clip)
	{
		yield return new WaitForSeconds(timeforclip);
		...
		if(audioSource.clip.name == "1clip")
		{
	...
	protected void Update()
	{
		if(StartRotating == 1)
		{
			//Vector3 lTargetDir = TargetPlanet.transform.position - Ship.transform.position;
			//lTargetDir.y = 0.0f;
			//Ship.transform.rotation = Quaternion.RotateTowards(Ship.transform.rotation, Quaternion.LookRotation(lTargetDir), Time.time * 0.01f);
		}
	}
}
